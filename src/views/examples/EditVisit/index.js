...
const EditVisit = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      // Load dos labs
      const { data: laboratories } = await api.get('laboratories');

      const data = laboratories.map((role) => ({
        value: role.id,
        name: role.name
      }));
      ...
      //Load da visita
      const { data: visit } = await api.get(`/visits/${id}`);

      setLab(visit.laboratory_id);
      setVisitorName(visit.visitor_name);
      setVisitorDocument(visit.visitor_document);
